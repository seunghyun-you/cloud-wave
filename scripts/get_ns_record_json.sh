...
# mapfile -t NS_RECORDS < <(aws route53 list-resource-record-sets --hosted-zone-id "$ZONE_ID" --query "ResourceRecordSets[?Type == 'NS' && Name == '$RECORD_NAME.$ZONE_NAME'].ResourceRecords[].Value" --output text)
# read -r ns1 ns2 ns3 ns4 <<< "$NS_RECORDS"
...
# sed -e "s|\"Name\": \".*\"|\"Name\": \"$recordName\"|g" \
#     -e "s|\"Value\": \".*\"|\"Value\": \"$ns1\"|1" \
#     -e "s|\"Value\": \".*\"|\"Value\": \"$ns2\"|1" \
#     -e "s|\"Value\": \".*\"|\"Value\": \"$ns3\"|1" \
#     -e "s|\"Value\": \".*\"|\"Value\": \"$ns4\"|1" \
#     "$JSON_DIR" > temp.json && mv temp.json "$JSON_DIR"